...
parameters {
  ...
  real<lower=0.0> eco[N_openings];
  real<lower=0.0> eco_alpha;
  real<lower=0.0> eco_beta;
}

transformed parameters {
  vector[N_games] den; // Denominator for the calculation of probabilities
  vector[3] alpha[N_games]; // Game result probabilities
  real nu = eco_alpha/eco_beta;  // Expose the mean of the r.e. distribution

  for (g in 1:N_games){
    den[g] = rating[white[g]] + rating[black[g]] + eco[opening[g]] * sqrt(rating[white[g]] * rating[black[g]]);
...
model {
  ...
  eco_alpha ~ normal(0, 10);
  eco_beta ~ normal(0, 10);
  eco ~ gamma(eco_alpha, eco_beta);
